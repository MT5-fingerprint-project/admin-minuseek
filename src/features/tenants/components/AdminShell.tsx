...
export function AdminShell({ children, sidebarNavigation }: AdminShellProps) {
  const { t } = useTranslation()
  const { email, username, logout } = useAuth()

  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r bg-card px-5 py-6 lg:flex lg:flex-col">
        ...
        <nav className="mt-8 flex min-h-0 flex-1 flex-col" aria-label={t('navigation.tenants')}>
          {sidebarNavigation ?? (
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-10 border-b bg-background/95 px-5 py-4 backdrop-blur">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
            ...
            <Button className="lg:hidden" variant="outline" onClick={logout}>
              <LogOut className="size-4" aria-hidden="true" />
              {t('auth.logout')}
            </Button>
          </div>
